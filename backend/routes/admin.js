...
//     // ✅ Cloudinary automatically gives you a secure URL (req.file.path)
//     const newAssignment = new Assignment({
//       title,
//       department,
//       semester,
...
//   cloudinary,
//   params: async (req, file) => ({
//     folder: "uploads", // common folder for all files
//     resource_type: "raw", // supports pdf, jpg, png, etc.
//     allowed_formats: ["pdf", "jpg", "jpeg", "png"],
//     public_id: Date.now() + "-" + file.originalname,
...
router.post("/pyqs", upload.single("file"), async (req, res) => {
  try {
    ...
    console.error("Add PYQ Error:", err);
    res.status(500).json({ success: false, message: "Failed to add PYQ" });
  }
});
